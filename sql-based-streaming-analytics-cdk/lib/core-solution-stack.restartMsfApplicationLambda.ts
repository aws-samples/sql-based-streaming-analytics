...
export async function handler(event: S3Event) {
    ...
    for (const record of event.Records) {
        ...
        if (msfApplicationToUpdate) {
            let msfAppDetails = await msfClient.send(new DescribeApplicationCommand({
            ...
            let existingPropertyGroups = msfAppDetails.ApplicationDetail?.ApplicationConfigurationDescription?.EnvironmentPropertyDescriptions?.PropertyGroupDescriptions;
            ...
            await msfClient.send(new UpdateApplicationCommand({
                ApplicationName: msfApplicationToUpdate.ApplicationName,
